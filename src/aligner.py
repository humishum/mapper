...
class Aligner:
    ...
    def align(self, method: str = "sequential_icp") -> list[o3d.geometry.PointCloud]:
        ...
        if method == "sequential_icp":
            ...
            aligned_point_clouds = self._align_sequential_point_to_plane(
                self.point_clouds
            )
    ...
    def _align_sequential_icp(
        ...
        for i in tqdm.tqdm(range(len(point_clouds) - 1)):
            ...
            source_down, target_down, source_fpfh, target_fpfh = prepare_dataset(
                self.voxel_size, source, target
            )
            initial_global_registration = execute_global_registration(
            ...
            icp_result = o3d.pipelines.registration.registration_icp(
                ...
                target,
                self.threshold,
                initial_global_registration.transformation,
                o3d.pipelines.registration.TransformationEstimationPointToPoint(),
            )
            point_clouds[i + 1] = point_clouds[i + 1].transform(
                icp_result.transformation
            )

        return point_clouds

    def _align_sequential_point_to_plane(
        self, point_clouds: list[o3d.geometry.PointCloud]
    ) -> list[o3d.geometry.PointCloud]:
...
def preprocess_point_cloud(pcd, voxel_size):
    logger.info(
        f"Downsample with a voxel size {voxel_size} (points: {len(pcd.points)})"
    )
    ...
    pcd_fpfh = o3d.pipelines.registration.compute_fpfh_feature(
        pcd_down, o3d.geometry.KDTreeSearchParamHybrid(radius=radius_feature, max_nn=50)
    )
...
def run_alignment_wrapper(args):
    ...
    print(
        f"Starting alignment: folder={folder_path}, output={output_path}, voxel_size={voxel_size}, method={method}"
    )
    time_start = time.time()

    try:
        aligner = Aligner(
            folder_path, output_path, threshold=threshold, voxel_size=voxel_size
        )
