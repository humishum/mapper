...
class DA3StreamingModel(BaseModel):
    ...
    def _load_combined_ply(self, ply_path: Path) -> PointCloud:
        ...
        if ply_path.exists():
            ...
            pointcloud.is_metric = True
            return pointcloud
        logger.warning("DA3 combined point cloud not found: %s", ply_path)
        return PointCloud(
    ...
    def _resolve_da3_dir(self) -> Path:
        ...
        sibling = repo_root / "da3-streaming"
        da3_dir = self._locate_da3_dir(sibling)
        if da3_dir is not None:
            return da3_dir
