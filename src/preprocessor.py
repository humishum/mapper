...
class Preprocessor:
    ...
    def _check_cached(self):
        ...
        if self.metadata_path.exists():
            ...
            if (
                ...
                and metadata["video_name"] == self.video_path.name
            ):
                logger.info(f"Frames already cached at {self.output_path}")
                return True
        else:
            logger.info(
                f"No metadata file found at {self.metadata_path}, will need to process video"
            )
            return False
    ...
    def _check_exiftool_installed(self):
        try:
            ...
            raise RuntimeError(
                "ExifTool is not installed. Please install ExifTool and try again."
            )
    ...
    def _check_hardware_acceleration(self):
        ...
        try:
            ...
            if any(accel in available_hwaccels for accel in ["cuda", "nvdec", "nvenc"]):
                ...
                accel in available_hwaccels for accel in ["vaapi", "videotoolbox"]
            ):
    ...
    def _get_video_duration(self) -> float:
        ...
        try:
            ...
            logger.warning(
                "Could not determine video duration, progress bar will be less accurate"
            )
            return 0.0
    ...
    def _get_frames(self):
        ...
        cmd.extend(
            [
                ...
                str(self.video_path),
                "-vf",
                f"fps={FPS}",  # Use fps filter for better performance than -r
                # "-an -sn -dn", # skip audio and other streams
                # "-c:v", "mjpeg",  # Use MJPEG encoder for faster JPEG encoding
                "-q:v",
                str(JPEG_QUALITY),  # JPEG quality (1-31, lower = higher quality)
            ...
            while True:
                ...
                try:
                    ...
                    if time.time() - last_update_time > 30:
                        logger.warning(
                            "No progress updates for 30 seconds, continuing to wait..."
                        )
                        last_update_time = time.time()
    ...
    def _save_metadata(self) -> dict:
        ...
        metadata = {
            ...
            "camera_focal_length_35mm_equivalent": (
                self.exif_data.get("QuickTime:CameraFocalLength35mmEquivalent") or None
            ),
